class NeuralNetwork {
  ...
  //All biases
  float[] biases = {0,0,0,0,0,0,0};//new float[7];
  ...
  float getOutput(float x1, float x2, float x3, float y1, float y2, float y3, float z1, float z2, float z3) {
    //layer1
    float o11 = weights[0]*x1+ weights[1]*x2 + weights[2]*x3 + biases[0];
    float o12 = weights[3]*x1+ weights[4]*x2 + weights[5]*x3 + biases[1];

    float o13 = weights[6]*y1+ weights[7]*y2 + weights[8]*y3 + biases[2];
    float o14 = weights[9]*y1+ weights[10]*y2 + weights[11]*y3 + biases[3];

    float o15 = weights[12]*z1+ weights[13]*z2 + weights[14]*z3 + biases[4];
    float o16 = weights[15]*z1+ weights[16]*z2 + weights[17]*z3 + biases[5];

    //layer2
    return o11*weights[18] + o12*weights[19] + o13*weights[20] + o14*weights[21] + o15*weights[22] + o16*weights[23] + biases[6];
  }
}
